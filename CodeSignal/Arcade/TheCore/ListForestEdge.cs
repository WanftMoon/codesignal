...
namespace CodeSignal.Arcade.TheCore
{
    public class ListForestEdge
    {
        ...
        {
            return inputArray.Where((_, i) => i < l || i > r).ToArray();
        }
        ...
        {
            if (arr[0] == arr[arr.Length - 1] && arr[0] == Middle(arr))
                return (true);
        ...
        public static int[] replaceMiddle(int[] arr)
        {
            ...
            var lst = arr.Where((_, i) => i < (arr.Length / 2 - 1) || i > (arr.Length / 2)).ToList();

            lst.Insert(arr.Length / 2 - 1, middle);
